...
import static ca.sirdanieliii.SD_SMP.SD_SMP.getThisPlugin;
...
public class ConfigYML implements Config {
    ...
    public ConfigYML(String path, String filename, String referencePath, String referenceFile) {
        ...
        if (!file.exists()) {
            if (getThisPlugin().getResource(referencePath + referenceFile + ".yml") != null) {
                ...
                if (!new File(getThisPlugin().getDataFolder(), referencePath + referenceFile + ".yml").renameTo(file)) {
                    getThisPlugin().getLogger().warning(String.format("Could not write file: %s", path + filename + ".yml"));
                }
            } else {
                try {
    ...
    public void sortKeys(String path) {
        try {
            reload();
            // Parse original keys & data
            Set<String> keys = Objects.requireNonNull(getConfig().getConfigurationSection(path)).getKeys(false);
            HashMap<String, Object> originalData = new HashMap<>();
            for (String key : keys) {
                originalData.put(key, getConfig().get(path + "." + key));
            }
            // Sort keys
            try {
                List<Integer> numberKeys = new ArrayList<>();
                for (String i : keys) {
                    numberKeys.add(Integer.parseInt(i));
                }
                Collections.sort(numberKeys);
                // Delete original values
                getConfig().set(path, new ArrayList<>());
                // Write original values, but in sorted order
                for (Integer key : numberKeys) {
                    getConfig().set(path + "." + key, originalData.get(key.toString()));
                }
                save();
                ...
                for (String key : sortedKeys) {
                    getConfig().set(path + "." + key, originalData.get(key));
                }
                save();
            }
    ...
    public void sortKeys(String path, String newKey) {
        try {
            reload();
            // Parse original keys & data
            Set<String> keys = Objects.requireNonNull(getConfig().getConfigurationSection(path)).getKeys(false);
            ...
            // Add new key
            keys.add(newKey);
            originalData.put(newKey, new ArrayList<>());
            // Sort keys
            List<String> sortedKeys = new ArrayList<>(keys);
            Collections.sort(sortedKeys);
            // Delete original values
