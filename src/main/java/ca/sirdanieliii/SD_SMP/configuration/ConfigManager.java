...
public class ConfigManager {
    ...
    public static Map<String, String> generalMsgs = new HashMap<>();

    public static void setupPlayerConfig(Player player) {
        ConfigPlayer configPlayer = new ConfigPlayer("/playerdata/", player.getUniqueId().toString(), "", "default_player_config");
        if (!configPlayer.update(player)) {
            SD_SMP.getThisPlugin().getLogger().info(String.format("Unable to update %s", configPlayer.getFile().getName()));
        } else {
            configPlayer.reload();
            configPlayer.updateValuesOnJoin(player);
    ...
    public static void reloadConfigs() {
        ...
        updateConfigs(config, errorMsgs, customMsgs);

        Bukkit.getOnlinePlayers().forEach(ConfigManager::setupPlayerConfig);

        loadConfigValues(config);
        loadErrorMessages(errorMsgs);
        loadCustomMessages(customMsgs);
    }

    private static void updateConfigs(ConfigYML... configs) {
        for (ConfigYML config : configs) {
            if (!config.update()) {
                SD_SMP.getThisPlugin().getLogger().severe(String.format("Unable to update %s! Problems may occur", config.getFile().getName()));
            }
        }
    }

    private static void loadConfigValues(ConfigMain config) {
        ...
        welcomeEnable = config.getConfig().getBoolean("welcome.enabled");
        welcomeFadeIn = config.getConfig().getInt("welcome.fade_in");
        welcomeStay = config.getConfig().getInt("welcome.stay");
        welcomeFadeOut = config.getConfig().getInt("welcome.fade_out");
    ...
    private static void loadErrorMessages(ConfigErrors errorMsgs) {
    ...
    private static void loadCustomMessages(ConfigMessages customMsgs) {
