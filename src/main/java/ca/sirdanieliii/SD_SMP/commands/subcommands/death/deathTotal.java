...
public class deathTotal extends SubCommand {
    ...
    public boolean perform(Player player, String[] args) {
        ...
        int total = player.getStatistic(Statistic.DEATHS);
        if (total == 0) player.sendMessage(Utilities.translateMsgClr(CommandManager.cmdHeader("death") + "&7You have never died before :O"));
        else if (total == 1) player.sendMessage(Utilities.translateMsgClr(CommandManager.cmdHeader("death") + "&FYou have died &Eonly once &Fin total!"));
        else player.sendMessage(Utilities.translateMsgClr(CommandManager.cmdHeader("death") + "&FYou have died " + "&C" + total + " times &Fin total! Imagine."));
        return true;
    }
